...
func NewiCloudPhotoImporter(ctx context.Context, name string, config map[string]string) (importer.Importer, error) {
...
func checkIcloudpd() error {
	if os.Getenv("PATH") == "" {
		_ = os.Setenv("PATH", "/usr/local/sbin:/usr/local/bin:/usr/bin:/bin")
	}

	_, err := exec.LookPath("icloudpd")
	if err != nil {
		return fmt.Errorf("icloudpd is not installed or not in PATH: %w", err)
	}
	return nil
}
...
func main() {
	...
	icloudImporter, err := NewiCloudPhotoImporter(kcontext.NewKContext(), "iphoto", scanMap)
	if err != nil {
		panic(err)
	}
